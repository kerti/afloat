...
whole lifecycle: one database per backend from `db/migrations`, both servers
started as plain processes on their §1 ports, a readiness wait on
`GET /api/health`, and a teardown that prints both logs if anything failed.
...
Today the list holds two permanent entries — `Date` and `Content-Length`, both
properties of HTTP rather than decisions — and **eight provisional ones, all
